...
class state:
    ...
    def getOutput(self, inputId):
        if inputId in self.inputSymbol:
            ...
            # for i in self.oedge:
            #    print str(i.cur_state)+str(i.to_state)
            # print inputId
            return -1,-1
        if pos == -1:
            sys.exit(1)
...
class stateMachine:
    ...
    def runFst_oneLine(self,line):
        if(len(line.split())==2):
            ...
            # print self.keyword_id
            # for i in range(len(self.prev_tokens)):
            #     print i
            #     print self.prev_tokens[i].score
            if self.spot==False:
                # print self.prev_tokens[self.endState_id-1].score
                self.postList[self.name]=np.exp(self.prev_tokens[self.endState_id].average_keyword_score)
            # print self.postList[self.name]
