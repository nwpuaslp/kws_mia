...
echo $#
if [ $# != 7 ]; then
   echo "Usage: $0 <data-train> <data-dev> <data-test> <lang-dir> <ali-train> <ali-dev> <exp-dir>"
   echo " e.g.: $0 data/train data/cv data/test data/lang exp/mono_ali_train exp/mono_ali_cv exp/mono_nnet"
   echo ""
...
if [ ! -z "$labels" ]; then
  ...
  labels_tr="ark:ali-to-phones --per-frame=true $alidir/final.mdl \\\"ark:gunzip -c $alidir/ali.*.gz |\\\" ark:- | utils/apply_map.pl $phone_map_file - - | ali-to-post ark:- ark:- |"
  labels_cv="ark:ali-to-phones --per-frame=true $alidir_cv/final.mdl \\\"ark:gunzip -c $alidir_cv/ali.*.gz |\\\" ark:- | utils/apply_map.pl $phone_map_file - - | ali-to-post ark:- ark:- |"
fi
...
if [ "$copy_feats" == "true" ]; then
  ...
  copy-feats scp:$data_test/feats.scp ark,scp:$tmpdir/test.ark,$dir/test.scp || exit 1
...
if [ ! -z "$cmvn_opts" ]; then
  if [ ! -z $global_cmvn_file ]; then
    ...
    feats_test="$feats_test apply-cmvn $cmvn_opts --utt2spk=ark:$data_test/utt2spk scp:$data_test/cmvn.scp ark:- ark:- |"
  fi
else
